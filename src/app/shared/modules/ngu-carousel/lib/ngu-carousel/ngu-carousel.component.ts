...
export class NguCarousel<T> extends NguCarouselStore
  ...
  private _pauseController$ = new Subject<number>();
  ...
  /**Used to pause the carousel */
  public pause(): void {
    this.isPaused = true;
    this._pauseController$.next(0);
  }

  /**Used to continue the carousel scrolling */
  public continue(): void {
    this.isPaused = false;
    this._pauseController$.next(1);
  }
  /** Init carousel point */
        ...
        this.carouselInt = merge(
          ...
          .pipe(
            ...
            switchMap(val => {
              this.isHovered = !val;
              this.cdr.markForCheck();
              return (!this.isHovered && !this.isPaused) ? interval$ : empty();
            })
          )
          .subscribe(res => {
            this._carouselScrollOne(1);
